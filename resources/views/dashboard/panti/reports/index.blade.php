...
@section('page_description', 'Laporan kegiatan dari relawan untuk panti kamu.')

@section('content')

    @if ($reports->isEmpty())
        <x-empty-state icon="check-circle" title="Belum ada laporan" description="Laporan dari relawan setelah kunjungan akan muncul di sini." />
    @else
        <div class="space-y-4">
            @foreach ($reports as $report)
                <x-card>
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3">
                        <div>
                            <h3 class="font-bold text-stone-ink">{{ $report->user->name ?? 'Relawan' }}</h3>
                    ...
                    @if ($report->note)
                        <p class="mt-2 text-xs text-stone-gray">Catatan: {{ $report->note }}</p>
                    @endif
                </x-card>
            @endforeach
        </div>
